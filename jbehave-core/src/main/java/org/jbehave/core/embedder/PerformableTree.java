...
public class PerformableTree {
    ...
    private PerformableStory performableStory(RunContext context, Story story, Map<String, String> storyParameters) {
        ...
        if (storyAllowed) {
            ...
            for (Scenario scenario : story.getScenarios()) {
                ...
                PerformableScenario performableScenario = performableScenario(
                        new performableScenarioParamObj(context, story, scenarioParameters, filteredStory, storyMeta, runBeforeAndAfterScenarioSteps, scenario));
    ...
    private PerformableScenario performableScenario(performableScenarioParamObj performableScenarioParamObj) {
        PerformableScenario performableScenario = new PerformableScenario(performableScenarioParamObj.getScenario(), performableScenarioParamObj.getStory().getPath());
        // scenario also inherits meta from story
        boolean scenarioAllowed = true;
        if (failureOccurred(performableScenarioParamObj.getContext()) && performableScenarioParamObj.getContext().configuration().storyControls().skipScenariosAfterFailure()) {
            return performableScenario;
        }

        if (!performableScenarioParamObj.getFilterContext().allowed(performableScenarioParamObj.getScenario())) {
        ...
        if (scenarioAllowed) {
            Meta storyAndScenarioMeta = performableScenarioParamObj.getScenario().getMeta().inheritFrom(performableScenarioParamObj.getStoryMeta());
            NormalPerformableScenario normalScenario = normalScenario(performableScenarioParamObj.getContext(), performableScenarioParamObj.getStory(), performableScenarioParamObj.getScenario(), storyAndScenarioMeta,
                    performableScenarioParamObj.getStoryParameters());

            // run before scenario steps, if allowed
            if (performableScenarioParamObj.isRunBeforeAndAfterScenarioSteps()) {
            	normalScenario.addBeforeSteps(performableScenarioParamObj.getContext().beforeOrAfterScenarioSteps(storyAndScenarioMeta,
                        Stage.BEFORE, ScenarioType.NORMAL));
            }
            
			if (isParameterisedByExamples(performableScenarioParamObj.getScenario())) {
                ExamplesTable table = performableScenarioParamObj.getScenario().getExamplesTable();
                ...
                for (int exampleIndex = 0; exampleIndex < tableRows.size(); exampleIndex++) {
                    ...
                    Meta exampleScenarioMeta = parameterMeta(performableScenarioParamObj.getContext(), scenarioParameters).inheritFrom(storyAndScenarioMeta);
                    boolean exampleScenarioAllowed = performableScenarioParamObj.getContext().filter().allow(exampleScenarioMeta);

                    if (exampleScenarioAllowed) {
                        ExamplePerformableScenario exampleScenario = exampleScenario(performableScenarioParamObj.getContext(), performableScenarioParamObj.getStory(), performableScenarioParamObj.getScenario(),
            ...
            if (performableScenarioParamObj.isRunBeforeAndAfterScenarioSteps()) {
            	normalScenario.addAfterSteps(performableScenarioParamObj.getContext().beforeOrAfterScenarioSteps(storyAndScenarioMeta, Stage.AFTER,
                        ScenarioType.NORMAL));
            }
